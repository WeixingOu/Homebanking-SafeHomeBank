const LoanNotification = {
    data() {
        return {
            notifications: this.loadLNotificationsFromLocalStorage(),
        }
    },

    methods: {
        loadLNotificationsFromLocalStorage() {
            const storedNotifications = localStorage.getItem('notificationsL');
            return storedNotifications ? JSON.parse(storedNotifications) : [];
        },
        deleteLNotification(index) {
            this.notifications.splice(index, 1);
            localStorage.setItem('notificationsL', JSON.stringify(this.notifications));
        }
    },
    ...
    template: `
        <div class="col-12 grid-margin stretch-card mt-3">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title m-0">Loan Notification</h4>
                </div>
                ...
                <ul class="icon-data-list" v-if="notifications.length">
                    <li class="position-relative" v-for="notification in notifications">
                        ...
                        <div>
                            <button @click="deleteLNotification(index)" class="btn-bi-x">
